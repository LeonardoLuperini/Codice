...
#ifndef EXIT_ERROR
#define EXIT_ERROR(val, errval, msg)                                           \
    if ((val) == (errval)) {                                                   \
        perror((msg));                                                         \
        exit(EXIT_FAILURE);                                                    \
    }
...
static void get_cli_input(int argc, char **argv, char *path, size_t *W,
                          size_t *N);
...
int main(int argc, char *argv[]) {
    ...
    bb = mmap(NULL, sizeof(bb_t) + N * MAX_NAME_LEN * sizeof(char),
...
void scanfs(char *path, bb_t *bb) {
    ...
    EXIT_ERROR(d = opendir(path), NULL, "Error opendir");
    while ((errno = 0, file = readdir(d)) != NULL) {
        if (NOT_PREV_DIR(file->d_name) && NOT_THIS_DIR(file->d_name) &&
            IS_DIR(file)) {
            concat_path(path, file->d_name, newpath);
            scanfs(newpath, bb);
        }
...
void collector(int p[]) {
    ...
    while (true) {
        if (readpipe(p[0], &d, sizeof(data_t)))
            break;
